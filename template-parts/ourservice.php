<section class="services-section" id="ourservice">
  <div class="sub-services-section">
  <div class="services-header">
    <h2>Our Services</h2>
    <p>Supplying premium groceries, seafood, and frozen items to banquets and restaurants.</p>
  </div>

  <div class="services-intro">
    <p class="headline">We specialize in delivering <em>freshness</em> and <strong>reliability</strong> to your kitchen.</p>
  </div>

  <div class="services-grid">
    <div class="service-card">
      <img src="<?php echo get_template_directory_uri(); ?>/assets/images/grocery.png" alt="Banquets">
      <h3>Banquet Supply</h3>
      <p>Bulk supply solutions tailored for large events and banquet kitchens.</p>
    </div>
    <div class="service-card">
      <img src="<?php echo get_template_directory_uri(); ?>/assets/images/grocery.png" alt="Restaurants">
      <h3>Restaurant Support</h3>
      <p>Customized inventory plans for consistent restaurant operations.</p>
    </div>
    <div class="service-card">
      <img src="<?php echo get_template_directory_uri(); ?>/assets/images/grocery.png" alt="Logistics">
      <h3>Cold Chain Logistics</h3>
      <p>Reliable and timely cold storage delivery to preserve freshness.</p>
    </div>
  </div>
  </div>
</section>
